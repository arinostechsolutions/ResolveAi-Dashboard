...
import { ComparisonData } from "@/hooks/use-analytics";

type EngagementComparisonChartProps = {
  data: ComparisonData[];
  isLoading?: boolean;
  compareBy: "neighborhood" | "type";
};
...
export function EngagementComparisonChart({
  ...
  const chartData = data.slice(0, 10).map((item) => ({
    ...
    engagement: Number(item.avgEngagement.toFixed(1)),
    ...
    >
      <header className="mb-4 sm:mb-6">
        ...
        <p className="mt-1 text-xs text-slate-400 sm:text-sm">
          Engajamento médio por {compareBy === "neighborhood" ? "bairro" : "tipo"} (top 10)
        </p>
      </header>
            ...
            <BarChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 60 }}>
              ...
              <Tooltip
                ...
                formatter={(value: number, name: string, props: any) => {
                  if (name === "engagement") {
                    return [`${value} pontos`, "Engajamento Médio"];
                  }
                  if (name === "resolutionRate") {
                    return [`${value}%`, "Taxa de Resolução"];
                  }
                  return [value, name];
                }}
                labelFormatter={(value) => {
                  const item = chartData.find((d) => d.label === value);
                  return item?.fullLabel || value;
                }}
              />
              <Bar 
                dataKey="engagement" 
