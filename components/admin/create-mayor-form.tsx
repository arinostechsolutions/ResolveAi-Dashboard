...
type CreateMayorFormProps = {
  cityId: string;
  onSuccess: () => void;
  onCancel: () => void;
};

type CreateMayorPayload = {
  name: string;
  email?: string;
  cpf?: string;
  phone: string;
...
async function createMayor(payload: CreateMayorPayload) {
  const response = await apiClient.post("/api/admin/users/mayor", payload);
  return response.data;
}

export function CreateMayorForm({
  cityId,
  onSuccess,
  onCancel,
}: CreateMayorFormProps) {
  ...
  const createMutation = useMutation({
    mutationFn: createMayor,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["adminUsers", cityId] });
      toast.success("Prefeito criado com sucesso!");
      setName("");
      setEmail("");
      setCpf("");
      setPhone("");
    ...
    onError: (error: any) => {
      const message =
        error.response?.data?.message || "Erro ao criar prefeito.";
      toast.error(message);
    },
  });
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            O prefeito terá acesso completo ao dashboard, mas apenas para esta cidade.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {createMutation.isPending ? "Criando..." : "Criar Prefeito"}
          </button>
          <button
            type="button"
            onClick={onCancel}
